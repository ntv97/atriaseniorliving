...
import (
	...
	"github.com/ntv97/atriaseniorliving/internal/cook/app"
...
func main() {
	...
	a.WaitstaffOrderPub.Configure(
		...
		pkgPublisher.MessageTypeName("cook-order-updated"),
	)

	a.Consumer.Configure(
		pkgConsumer.ExchangeName("cook-order-exchange"),
		pkgConsumer.QueueName("cook-order-queue"),
		pkgConsumer.BindingKey("cook-order-routing-key"),
		pkgConsumer.ConsumerTag("cook-order-consumer"),
	)
